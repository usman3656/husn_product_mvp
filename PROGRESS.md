...
| Field | Value |
|---|---|
| Last commit on `main` | `e5f796d` — GitHub Pages static demo (output: export + DEMO_MODE, baked Project Atlas data); committed locally, push to GitHub pending network |
...
- **2026-05-28** — Commit `e5f796d`: GitHub Pages static demo. Next.js `output: export` snapshot gated by `NEXT_OUTPUT_EXPORT=1` (basePath `/husn_product_mvp`), `DEMO_MODE` (`NEXT_PUBLIC_DEMO_MODE=1`) hides backend-bound controls (disconnect buttons, allowlist editors, "Run analysis") and bakes a canned chat transcript + connections snapshot; real Project Atlas graph data baked into the dashboard at build time by running `next build` inside the web container against the live API. Internal nav switched to `next/link` for basePath. `scripts/deploy-pages.sh` builds + force-pushes `web/out/` (with `.nojekyll`) to the `gh-pages` branch. Build verified green (6 routes, ATLAS-* + "Project Atlas" present in baked `index.html`). **NOT YET PUSHED** — github.com:443 unreachable from this machine; `gh-pages` committed locally in `web/out/.git`, awaiting connectivity. Serve at https://usman3656.github.io/husn_product_mvp/ once pushed + Pages enabled (branch gh-pages / root).
